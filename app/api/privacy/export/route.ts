...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // NOTE: RLS likely restricts order_items; if policy requires owner join, we can’t filter server-side in PostgREST.
    // Workaround: fetch then filter in memory (may be large; acceptable for privacy export scale).
    let order_items: any[] = [];
    if (orders.length > 0) {
